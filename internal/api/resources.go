...
func (s *Server) handleListDKIM(w http.ResponseWriter, r *http.Request) {
	snap, err := core.LoadSnapshot()
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "failed to load DKIM data"})
		return
	}

	records, err := core.ListDKIMDNSRecords(snap)
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "failed to list DKIM"})
		return
	}
	...
	domain := r.URL.Query().Get("domain")
	if domain != "" {
		filtered := make([]core.DKIMDNSRecord, 0)
...
func (s *Server) handleGenerateDKIM(w http.ResponseWriter, r *http.Request) {
	...
	if req.Domain == "" || req.Selector == "" {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "domain and selector required"})
		return
	}

	if err := core.GenerateDKIM(req.Domain, req.Selector); err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": err.Error()})
		return
	}

	// Reload and return the new record
	snap, _ := core.LoadSnapshot()
	records, _ := core.ListDKIMDNSRecords(snap)

	for _, rec := range records {
		if rec.Domain == req.Domain && rec.Selector == req.Selector {
